...
Original file is located at
    https://colab.research.google.com/drive/1ty3-uzSi-7L8u79j686XtrPBTkE0BbBZ
...
gray_like = build_image_like_tensor(1,1,3,127)
...
patriot = build_image_like_tensor(12,12,3,0)
patriot[:,0:4,:] = (0,0,255)
patriot[:,4:8,:] = (255,255,255)
patriot[:,8:12,:] = (255,0,0)
...
for i in range(5) :
  for j in range(5) :
    R_1[i,j] = sum(sum(K_1*cal[i:i+3,j:j+3]))
print(R_1)
...
def convolution_forward_numpy(image, kernel):
    ...
    [x,y] = image.shape
    ret = np.zeros((x,y))
    cal = np.zeros((x+2,y+2))
    cal[1:x+1,1:y+1] = image
    for i in range(x) :
      for j in range(y) :
        ret[i,j] = sum(sum(kernel*cal[i:i+3,j:j+3]))
...
# YOUR CODE HERE
output_image = convolution_forward_numpy(image[:,:,2], horizontal_kernel) 
...
print(torch.randn(2,2,2,2))

def convolution_forward_torch(image, kernel):
    # YOUR CODE HERE 
    m = nn.Conv2d(1, 1, 3, padding=1, bias=False)
    kernel = kernel.float()
    m.weight = torch.nn.Parameter( kernel )
    image = image.float()
    return m(image)

#K_1 = torch.from_numpy(K_1.astype(np.int32))
#K_1 = torch.unsqueeze(K_1, 0)
#K_1 = torch.unsqueeze(K_1, 0)
#im = torch.from_numpy(image[:,:,2].astype(np.int32))
#im = torch.unsqueeze(im, 0)
#im = torch.unsqueeze(im, 0)
print(convolution_forward_torch(im, K_1))
...
images = next(iter(fmnist_train))
print(images)

def plot_multiple(image, name) :
  plt.figure()
  plt.imshow(image, cmap='gray')
  plt.title(f"This is a {name}")

def display_10_images(dataset):
    # YOUR CODE HERE 
    images = next(iter(dataset))
    for i in range(10) :
      plot_multiple(images[0][i,:,:,:][0], images[1][i])
...
class CNNModel(nn.Module):
    def __init__(self, classes=10):
        ...
        self.conv1 = torch.nn.Conv2d(1,42,kernel_size=5,padding = 2)
        self.conv2 = torch.nn.Conv2d(42,69,kernel_size=5,padding = 2)

        self.conv3 = torch.nn.Conv2d(69,42,kernel_size=3,padding = 1)
        self.conv4 = torch.nn.Conv2d(42,13,kernel_size=3,padding = 1)
        self.conv5 = torch.nn.Conv2d(13,7,kernel_size=3,padding = 1)
...
if __name__ == "__main__":
    ...
    fmnist_train = FashionMNIST(os.getcwd(), train=True, download=True, transform=transforms.ToTensor())
    fmnist_train = DataLoader(fmnist_train, batch_size=50, num_workers=4, pin_memory=True)
    fmnist_val = FashionMNIST(os.getcwd(), train=False, download=True, transform=transforms.ToTensor())
    fmnist_val = DataLoader(fmnist_val, batch_size=50, num_workers=4,  pin_memory=True)
    # Train for an number of epoch 
    for epoch in range(nepoch):
      print(f"training Epoch: {epoch}")
      if epoch > 0:
